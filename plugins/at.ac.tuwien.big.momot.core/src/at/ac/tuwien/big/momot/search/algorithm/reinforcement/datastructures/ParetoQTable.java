...
public class ParetoQTable<S, A> extends QTable<S, A, ParetoQState> implements IParetoQTableAccessor<S, A> {
   ...
   private A _getMaxRewardAction(final EvaluationStrategy strategy, final double[] ref, final S state, final Problem p,
         ...
         case HYPERVOLUME:
            ...
            try {
               if(ref == null) {
                  ...
                  throw new RuntimeException("Need to track nadir point for this first!");
                  // final NondominatedPopulation ndp = new NondominatedPopulation();
                  // ndp.add(new Solution(ref));
                  // final double[] ideal = { 20, 0, 0 };
                  // ndp.add(new Solution(ideal));
                  //
                  // hv = new Hypervolume(p, ndp);
               }
   ...
   @Override
   public A getMaxRewardAction(final EvaluationStrategy strategy, final double[] ref, final S s, final Problem p,
         final NondominatedPopulation qndp) {
      final S mState = this.stateModifier != null ? this.stateModifier.modify(s) : s;
      final A maxRewA = this._getMaxRewardAction(strategy, ref, mState, p, qndp);
